...
windowStack.prototype.getStyleFloatOf = function (propertyNameOf, JSpropertyNameOf) {
    ...
    try {
        ...
        try {
            dirtyValue = this.domObject.currentStyle.getAttribute(
                JSpropertyNameOf,
            ); /*JS object notation style keywords, not the CSS keywords!*/
        } catch (error) {
            cout(error.message, 2);
        }
    }
    var cleanValue = parseFloat(dirtyValue != null && dirtyValue.length ? dirtyValue : "0");
    return !isNaN(cleanValue) && cleanValue != -1
        ? cleanValue
        : 0; /*Note: Opera gives -1, not NaN for cleanValue when no number is found.*/
};
windowStack.prototype.dragEnable = function (event) {
    ...
    if (mousedWindow || this.mousedInsideWindow) {
        ...
        if (highX - this.leftLast < 25 && highY - this.topLast < 25) {
            /*Inivisible resize corner area for windows.*/
            this.resize = true;
        } else {
            this.resize = false;
        }
...
windowStack.prototype.drag = function (event) {
    if (this.movable) {
        ...
        if (this.resize) {
            var newWidth =
                this.domObject.clientWidth +
                event.clientX -
                this.leftLast -
                this.getStyleFloatOf("padding-left", "paddingLeft") -
                this.getStyleFloatOf("padding-right", "paddingRight");
            var newHeight =
                this.domObject.clientHeight +
                event.clientY -
                this.topLast -
                this.getStyleFloatOf("padding-top", "paddingTop") -
                this.getStyleFloatOf("padding-bottom", "paddingBottom");
            if (
                this.getStyleFloatOf("min-width", "minWidth") <= newWidth &&
                this.getStyleFloatOf("min-height", "minHeight") <= newHeight &&
                (this.getStyleFloatOf("max-width", "maxWidth") >= newWidth ||
                    this.getStyleFloatOf("max-width", "maxWidth") == 0) &&
                (this.getStyleFloatOf("max-height", "maxHeight") >= newHeight ||
                    this.getStyleFloatOf("max-height", "maxHeight") == 0)
            ) {
