    ...
    if (keyName.substring(0, 9) == "B64_SRAM_") {
        ...
        downloadDiv.appendChild(
            outputLocalStorageLink(
                ...
                base64(io.generateBlob(keyName.substring(4), base64_decode(findValue(keyName)))),
                keyName,
            ),
        );
    } else if (keyName.substring(0, 5) == "SRAM_") {
        ...
        downloadDiv2.appendChild(
            outputLocalStorageLink("Download RAW save data.", base64(convertToBinary(findValue(keyName))), keyName),
        );
        subContainer.appendChild(downloadDiv2);
        downloadDiv.appendChild(
            outputLocalStorageLink(
                "Download in import compatible format.",
                base64(io.generateBlob(keyName, convertToBinary(findValue(keyName)))),
                keyName,
            ),
        );
    } else {
        downloadDiv.appendChild(
            outputLocalStorageLink(
                "Download in import compatible format.",
                base64(io.generateBlob(keyName, JSON.stringify(findValue(keyName)))),
                keyName,
            ),
        );
    }
